<properties
   pageTitle="Beispiel für eine Anwendung REST-basierten Lebenszyklus | Microsoft Azure"
   description="Einer Microsoft Azure Service Fabric Stichprobe, die den Lebenszyklus der Anwendung mithilfe der Dienst Fabric REST-Schnittstelle anzeigt."
...
# <a name="rest-based-application-lifecycle-sample"></a>Beispiel für eine Anwendung REST-basierten Lebenszyklus

In diesem Beispiel veranschaulicht Lebenszyklus der Fabric Service-Anwendung durch die REST-API Anrufe an. Weitere Informationen für den Dienst Fabric Anwendungslebenszyklus finden Sie unter [Lebenszyklus Fabric Service-Anwendung](service-fabric-application-lifecycle.md).

In diesem Beispiel folgende Aktionen ausgeführt:

* Vorschriften die Stichprobe **WordCount 1.0.0** aus dem Paket WordCount-Anwendung, in dem Bild gespeichert.
* Zeigt die Liste der Anwendungstypen, wozu auch WordCount 1.0.0 an.
* Erstellt die Anwendung WordCount als **Fabric: / WordCount**.
* Zeigt die Liste der Anwendungen, wozu auch Fabric: / WordCount Version 1.0.0.
* Vorschriften der 1.1.0-Version des Beispiels WordCount aus dem **WordCountUpgrade** Anwendungspaket im Bild Store.
* Zeigt die Liste der Anwendungstypen, die sowohl die WordCount 1.0.0 **WordCount 1.1.0**enthält.
* Die Anwendung WordCount Version 1.1.0 aktualisiert.
* Zeigt die Liste der Anwendungen, die WordCount Version 1.1.0 enthält, aber nicht mehr WordCount Version 1.0.0 enthält.
* Löscht die WordCount-Anwendung.
* Zeigt die Liste der Anwendungen, die nicht mehr Fabric enthält: / WordCount.
* Unprovisions der 1.1.0-Version des Beispiels WordCount.
* Zeigt die Liste der Anwendungstypen, die WordCount 1.0.0 enthält, aber nicht mehr WordCount 1.1.0 enthält.
* Unprovisions der 1.0.0-Version des Beispiels WordCount.
* Zeigt die Liste der Anwendungstypen, die nicht mehr WordCount enthält.


## <a name="prerequisites"></a>Erforderliche Komponenten

In diesem Beispiel wird die [Stichprobe WordCount](http://aka.ms/servicefabricsamples) (finden Sie in den Beispielen **Überfordert** ) verwendet. Im Beispiel WordCount muss zuerst erstellt werden, und klicken Sie dann auf das Bild Store zwei Anwendungspakete kopiert werden müssen.

|Ordner|Beschreibung|
|------|-----------|
|WordCount|Die Anwendung, WordCount Stichprobe. Die Datei **ApplicationManifest.xml** enthält **ApplicationTypeVersion = "1.0.0"**.|
|WordCountUpgrade|Die Anwendung, WordCount Stichprobe. Die Datei ApplicationManifest.xml muss geändert werden, um **ApplicationTypeVersion = "1.1.0"** an, damit die Aktualisierung der Anwendung ausgeführt.|

Wenn Sie die Anwendungspakete erstellen und kopieren Sie sie in der Abbildung Store, gehen Sie folgendermaßen vor:

1. Kopieren Sie **C:\ServiceFabricSamples\Services\WordCount\WordCount\pkg\Debug** in **C:\Temp\WordCount**an. Dadurch wird das Paket WordCount Anwendung erstellt.
2. Kopieren Sie C:\Temp\WordCount in **C:\Temp\WordCountUpgrade**. Dadurch wird das Paket **WordCountUpgrade Anwendung** erstellt.
3. Öffnen Sie **C:\Temp\WordCountUpgrade\ApplicationManifest.xml** in einen Text-Editor ein.
4. Ändern Sie im **ApplicationManifest** Element das Attribut **ApplicationTypeVersion** in **"1.1.0"**ein.  Dadurch wird die Versionsnummer der Anwendung aktualisiert.
5. Speichern Sie die geänderte ApplicationManifest.xml-Datei ein.
6. Führen Sie das folgende PowerShell-Skript als Administrator, um die Anwendungen zum Store Bild kopieren:
...
Abschluss das PowerShell-Skript kann diese Anwendung ausführen.

## <a name="example"></a>Beispiel

Das folgende Beispiel veranschaulicht den Lebenszyklus Fabric Service-Anwendung.
