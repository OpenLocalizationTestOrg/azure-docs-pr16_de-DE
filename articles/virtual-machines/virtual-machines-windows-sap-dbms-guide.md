...
Es gibt einige SQL Server IaaS bestimmte Informationen, die Sie, bevor Sie fortfahren kennen sollten:
...
* Verschieben Sie die primäre Tempdb Datendatei(en) auf demselben logischen Laufwerk als die primäre Daten Datei(en) aus der SAP-Datenbank.
...
* Das Tool verlassen eine kleine Stub-Datei ein paar KB Größe auf die für den SQL Server definiert wurde virtuelle Festplatte/Datenträger Sicherungsdatei. **Diese Datei darf im Speicherort bleiben, da sie Sie erneut aus Azure-Speicher Wiederherstellen benötigt wird.**
...
Um zusätzliche Datenbanken in anderer Weise besteht darin, zusätzliche virtuelle Festplatten eines virtuellen Computers anfügen, die Sie zum Speichern von Sicherungskopien auf verwenden. In diesem Fall müssen Sie möchten sicherstellen, dass die virtuellen Festplatten nicht vollständige ausgeführt werden. Wenn dies der Fall ist, müssen Sie würde heben Sie die Bereitstellung der virtuellen Festplatte und Mal so zu sagen 'archivieren' und durch eine neue, leere virtuelle Festplatte ersetzen. Wenn Sie nach unten, dass der Pfad zugreifen, diese virtuelle Festplatten in separaten Azure-Speicher-Konten von denen beibehalten werden soll, die die virtuellen Festplatten durch die Datenbankdateien.

Zweite Möglichkeit besteht darin, ein großes virtuellen Computers verwenden, die viele virtuelle Festplatten angefügt werden können. Z. B. D14 mit 32VHDs. Verwenden Sie Speicher Leerzeichen, um eine flexible Umgebung zu erstellen, könnten Sie Freigaben erstellen, die dann als Sicherung Ziele für die verschiedenen DBMS-Servern verwendet werden.
 
Einige bewährte Methoden haben dokumentierten [hier](https://blogs.msdn.com/b/sqlcat/archive/2015/02/26/large-sql-server-database-backup-on-an-azure-vm-and-archiving.aspx) ebenfalls. 
...
Aber bei Verwendung von 'Microsoft SQL Server-Sicherung an Microsoft Azure-Tool' in älteren Versionen können Sie mehr als eine Datei Ziel definieren. Mit mehr als ein Ziel kann die Sicherung skalieren und der Durchsatz der Sicherung höher ist. Dies würde dann mehrere Dateien als auch in das Konto Azure-Speicher führen. In unseren Tests mit implementiert mehrere Datei Ziele eine auf jeden Fall den Durchsatz erreichen kann, die eine mit der Sicherungsdatei Erweiterungen erreichen konnte aus SQL Server 2012 SP1 CU4 auf. Sie sind auch nicht durch die Beschränkung 1 TB wie in die native Sicherung in Azure blockiert.

Jedoch Bedenken Sie, der Durchsatz ist auch hängt vom Standort des Kontos Speicher Azure Sie für die Sicherung verwenden. Das Speicher-Benutzerkonto in einem anderen Bereich zu suchen, als die virtuellen Computern, in ausgeführt werden eine Vorstellung möglicherweise. Z. B. können Sie die Konfiguration virtueller Computer in "Westen"-Europa ausgeführt, aber das Speicher-Konto, mit denen Sie in den Hintergrund von gegen in Nordamerika-Europa setzen. Die sicher haben Einfluss auf die Sicherungsmediendurchsatz und wahrscheinlich keinen Durchsatz von 150 MB/s generieren, wie es scheint in Fällen möglich sein, in dem die Zielspeicher und die virtuellen Computern im gleichen Landes-/ Datencenter ausgeführt werden.
...
* Die Sortierung der SQL Server-Instanz, in dem die virtuellen Computern angeboten in dem Azure Marketplace installiert ist kein die Sortierung, die SAP NetWeaver SQL Server-Instanz ausgeführt erfordert. Sie können die Sortierreihenfolge ändern, obwohl mit der erfahren Sie, wie im folgenden Abschnitt.
...
#### <a name="sql-server-log-shipping"></a>SQLServer protokollieren Versand
...
Datenbank: Spiegelung als unterstützte SAP (Siehe SAP-Hinweis [965908]) abhängig Failoverpartner in der Verbindungszeichenfolge SAP definieren. Für die Fälle Cross lokale wird davon ausgegangen, dass die zwei virtuellen Computern in der gleichen Domäne sind und Kontext des Benutzers zwei SQL Server-Instanzen unter ausgeführt werden auch Domänen-Benutzer sind und über die erforderlichen Berechtigungen in der verbindet zwei SQL Server-Instanzen. Daher unterscheidet sich die Einrichtung von in Azure-Datenbank Spiegelung nicht zwischen einer normalen lokalen Setup/Konfiguration.
...
* Die Verfügbarkeit Gruppe Zuhörer soll in Azure mit TCP/IP Endpunkten erstellt werden, die mit den virtuellen Computern zugewiesen werden die primären und sekundären Replikate der Verfügbarkeit Gruppe ausgeführt werden.
...
Es ist möglich, eine SQL Server AlwaysOn Availability Group über andere ebenfalls Azure Regionen bereitstellen. Dieses Feature wird die Nutzung der Azure-VNet Vnet-Konnektivität ([Weitere Details][virtual-networks-configure-vnet-to-vnet-connection]).
...
* Softwareprobleme in der SQL Server-Instanz
...
1. Planen Sie sorgfältig Ihre SAP-System Querformat in Azure, um die Datei Datenlayout und Azure Einschränkungen verteilen:
...
1. Nie Installieren von Software oder alle Dateien, die auf das Laufwerk D:\ Beibehaltung erfordern, wie es nicht dauerhafte ist und nichts auf diesem Laufwerk bei einem Windows-Neustart verloren setzen.
...
1. Verwenden Sie die höchste Datenbank Komprimierung möglich. Dies ist für SQL Server Seite Komprimierung.
1. Achten Sie mithilfe von SQL Server-Bilder aus dem Azure Marketplace. Wenn Sie eine SQL Server verwenden, müssen Sie die Sortierung Instanz vor der Neuinstallation alle NetWeaver SAP-System daran ändern.
...
SAP aktuell unterstützt SAP-ASE Version 16,0 zur Verwendung mit SAP-Business Suite-Produkten. Alle Updates für ASE SAP-Server oder JDBC und ODBC-Treiber mit SAP-Business Suite-Produkten verwendet werden, werden allein durch SAP Service Marketplace am bereitgestellt: <https://support.sap.com/swdc>.
...
Bei einem bestimmten ERP oder alle BW Auslastung ist es sinnvoll, hinsichtlich der Leistung, bleiben die Tempdb Geräte für die darüber hinaus erstellten Tempdb (durch SWPM oder manuell) auf einem anderen Laufwerk als C:\. ist keine zusätzliche Tempdb vorhanden, es ist wird empfohlen, eine (SAP-Hinweis [1752266]) erstellen.
...
* Hinzufügen von Tempdb Geräte zu den einzelnen der virtuellen Festplatten mit einer Geräte der SAP-Datenbank
...
#### <a name="impact-of-database-compression"></a>Auswirkung der Datenbank Komprimierung
In Konfigurationen, e/a-Bandbreite eine Beschränkung verwendet werden kann, kann jeder die IOPS reduziert Measure hilfreich die Arbeitsbelastung Strecken, die einen in einem Szenario IaaS wie Azure ausgeführt werden kann. Es wird daher dringend empfohlen, um sicherzustellen, dass SAP-ASE Komprimierung vor dem Hochladen einer vorhandenen SAP-Datenbank in Azure verwendet wird.
...
* Die Menge der Daten auf Azure hochgeladen werden ist niedriger.
...
Weitere Details im Zusammenhang mit der DNS-Name finden Sie [hier][virtual-machines-azurerm-versus-azuresm].
...
Zusätzlich zu Daten – und der BRANCHENDATEN bietet Komprimierung SAP-ASE auch Komprimierung. Um weniger Platz für die Datenbank und der Protokolldateien speichert empfiehlt es sich Komprimierung verwenden. Finden Sie SAP-Hinweis [1588316] Weitere Informationen. Komprimieren die Sicherung ist entscheidend, verringern die Datenmenge übertragen werden, wenn Sie beabsichtigen, Sicherungskopien oder virtuelle, enthält die Sicherungsdatei speichert aus der Azure-virtuellen Computern zu lokalen Festplatten herunterladen.
...
* Weniger die Anzahl der virtuellen Festplatten verwendet zum Speichern der Datenbank-Geräte, desto geringer der Gesamtdurchsatz lesen
...
Die beiden folgenden SAP-Hinweise umfassen allgemeine Informationen zu ASE unter Linux und ASE in der Cloud:
...
#### <a name="structure-of-the-sap-ase-deployment"></a>Struktur der ASE SAP-Bereitstellung
Gemäß der allgemeinen Beschreibung werden ASE SAP-Programmdateien ansässig oder in Root-Dateisystem mit dem virtuellen Computer (/sybase) installiert. Die meisten Datenbanken ASE SAP-System und Tools in der Regel werden von SAP NetWeaver Arbeitsbelastung nicht wirklich harte eingesetzt werden. Daher können die System und Tools Datenbanken (Master-Shape, Modell, Saptools, Sybmgmtdb, Sybsystemdb) auf Root-Dateisystem auch bleiben. 
...
Oracle Data Guard wird für hohen Verfügbarkeit und Ausfall unterstützt. Details finden Sie in [dieser] [ virtual-machines-windows-classic-configure-oracle-data-guard] Dokumentation.
...
Sie finden die aktualisierte Liste der SAP MaxDB Dokumentation in die folgenden SAP-Hinweis [767598]
...
Azure-Speicher bewährte Methoden für SAP MaxDB führen Sie die allgemeinen Empfehlungen erwähnt in Kapitel [Struktur einer Bereitstellung RDBMS] [Dbms Leitfaden 2].
...
* Solange das Kontingent für die aktuelle IOPS pro Datenträger die Anforderungen erfüllt, ist es möglich, speichern alle Datenbestände auf einer einzelnen bereitgestellten Azure-virtuellen Festplatte, und speichern alle Datenbank Log Datenmengen auch auf einem anderen einzelnen bereitgestellten Azure virtuellen Festplatte.
...
Weitere allgemeinen Themen wie Azure Verfügbarkeit Sätze oder SAP-Überwachung gelten auch wie in diesem Dokument für die Bereitstellung von virtuellen Computern mit der Datenbank SAP MaxDB die ersten drei Kapitel beschrieben.
...
Für die Azure-virtuellen Computer-Typen von SAP (SAP-Hinweis [1928533]) unterstützt werden werden alle virtuelle CPU-Ressourcen für den virtuellen Computer reserviert von dedizierten physische CPU-Ressourcen des Hypervisors unterstützt. Keine Bereitstellung überproportional vieler (und daher keine induzierten Risikos für die CPU-Ressourcen) erfolgt.
...
Es wird dringend empfohlen, verwenden Sie die neueste Version von Microsoft Windows, die zum Zeitpunkt der Schreiben dieses Dokument **Windows Server 2012 R2**ist.
