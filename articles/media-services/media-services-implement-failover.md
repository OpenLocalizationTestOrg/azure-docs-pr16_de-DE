<properties 
    pageTitle="Implementieren der Failover streaming Szenario | Microsoft Azure" 
    description="In diesem Thema wird gezeigt, wie Failover streaming Szenario implementieren." 
...
#<a name="implementing-failover-streaming-scenario"></a>Implementieren der Failover streaming Szenario

Diese exemplarische Vorgehensweise veranschaulicht, wie Inhalt (Blobs) Kopieren von einer Anlage in ein anderes um redundante redundante bei Bedarf streaming zu behandeln. Dieses Szenario eignet sich für Kunden, deren CDN Failover zwischen zwei Datencenter bei einem Ausfall in einem unsere Data Center einrichten möchten.
Diese exemplarische Vorgehensweise verwendet Microsoft Azure Media Services SDK, Microsoft Azure Media Services REST-API und Azure-Speicher SDK, um zu veranschaulichen, die folgenden Aufgaben.

1. Richten Sie ein Konto Media-Dienste in "A Data Center".
1. Hochladen einer Datei Mezzanine in einer Quelle Anlage.
1. Codieren Sie die Anlage in Multi-bit Rate MP4-Dateien ein. 
1. Erstellen Sie einen schreibgeschützte SAS Locator für die Anlage Quelle zu Ihrem Konto Speicher Lesezugriff auf den Container vorhanden sein, die die Quelle Anlage zugeordnet ist.
1. Rufen Sie den Containername der Anlage Quelle in der schreibgeschützt SAS Locator im vorherigen Schritt erstellt. Wir benötigen diese Informationen für das Kopieren von Blobs zwischen Speicherkonten (später im Thema erläutert.)
1. Erstellen eines Origin Locators für die Anlage, die durch die Codierung Aufgabe erstellt wurde. 

Klicken Sie dann das Failover verarbeitet:

1. Richten Sie ein Konto Media-Dienste in "Data Center B" aus.
1. Erstellen einer leeren Ziel-Anlage in der Zielliste Media Services-Konto an.
1. Erstellen Sie einen schreiben SAS Locator für die Ziel-leeren Anlage über Schreibzugriff auf den Container in der Zielliste Speicher-Konto verfügen, die Ziel-Anlage zugeordnet ist.
1. Verwenden von Azure-Speicher SDK zum Kopieren von Blobs (Objektdateien) zwischen Speicher-Konto in "Data Center A" Quelle und Ziel-Speicher-Konto in "Data Center B" (diese Speicherkonten relevante Ressourcen zugeordnet sind.)
1. Zuordnen von Blobs (Objektdateien), die in der Zielliste Blob Container mit der Anlage Ziel kopiert wurden. 
1. Erstellen eines Origin Locators für die Anlage in "Data Center B", und geben Sie den Locator-Id, die für die Anlage in "Data Center A" generiert wurde. 
1. Dadurch werden die streaming URLs, wo sind die relativen Pfade URLs identisch (nur die Basis URLs unterscheiden). 
 
Klicken Sie dann können Sie zum Behandeln von einem beliebigen Ausfall eines CDN auf diese Origin Locator erstellen. 

Folgendes gilt:

- Die aktuelle Version von Media Services SDK unterstützt keine Locator mit einer angegebenen Locator-Id erstellen. Um diese Aufgabe zu erreichen, werden wir Media Services REST-API verwenden.
- Die aktuelle Version von Media Services SDK unterstützt keine programmgesteuert generieren von IAssetFile Informationen, die eine Anlage Anlage Dateien zuordnen möchten. Um diese Aufgabe zu erreichen, werden wir die CreateFileInfos Media Services REST-API verwenden. 
- Verschlüsselte Speicher Anlagen (AssetCreationOptions.StorageEncrypted) werden nicht für die Replikation unterstützt (da des Verschlüsselungsschlüssels in beide Konten Media-Dienste unterscheiden wird). 
- Wenn Sie dynamische Verpackung nutzen möchten, müssen Sie zuerst mindestens eine bei Bedarf Streaming reservierte Einheiten abrufen. Weitere Informationen finden Sie unter [Dynamisch Verpackung Posten](media-services-dynamic-packaging-overview.md).
 

>[AZURE.NOTE]Erwägen Sie das Media-Dienste [Replicator Tool](http://replicator.codeplex.com/) als Alternative zu einem Failover streaming Szenario manuell implementieren. Dieses Tool können Sie Posten über zwei Medien Dienstkonten repliziert.

##<a name="prerequisites"></a>Erforderliche Komponenten
 
- Zwei Media-Dienste-Konten in einem neuen oder vorhandenen Azure-Abonnement. Informationen Sie [zum Erstellen eines Media Services-Kontos](media-services-portal-create-account.md).
...
1. Verwenden Sie Visual Studio, um eine neue Lösung erstellen, die das Projekt c# Console Application enthält. Geben Sie HandleRedundancyForOnDemandStreaming ein, und klicken Sie dann auf OK.
1. Erstellen Sie den Ordner SupportFiles auf der gleichen Ebene wie die Projektdatei HandleRedundancyForOnDemandStreaming.csproj aus. Erstellen Sie unter dem Ordner SupportFiles Ordner Dateien und MP4Files ein. Kopieren einer MP4-Datei in den Ordner MP4Files (in diesem Beispiel wird die Datei BigBuckBunny.mp4 verwendet wird). 
...
1. Fügen Sie weitere Ressourcen, die für dieses Projekt erforderlich sind: System.Configuration Abschnittgruppe und System.Web.
...
##<a name="add-code-that-handles-redundancy-for-on-demand-streaming"></a>Hinzufügen von Code, der bei Bedarf streaming redundante redundante behandelt.



1. Die folgenden Felder der Klasse Ebene der Klasse Programm hinzufügen.
...
1. Ersetzen Sie die Definition der standardmäßigen primär Methode durch die folgende Regel:
...
1. Definitionen Methode, die über Main aufgerufen werden, werden unten definiert.
...
Sie können jetzt einen Datenverkehr-Manager die Weiterleitung von Anfragen zwischen zwei Data Centers und daher bei jeder Ausfall Failover verwenden.
