<properties
    pageTitle="Erweitern Sie Ihre Experimentieren mit R | Microsoft Azure"
    description="Wie Sie die Funktionalität der Azure maschinellen Learning Studio durch die Sprache R zu erweitern, mit dem Modul R Skript ausführen."
...
#<a name="extend-your-experiment-with-r"></a>Erweitern Sie Ihre Experimentieren mit R

Sie können die Funktionalität der ML Studio über die Sprache R erweitern, indem Sie mit dem [Ausführen R Skript] [ execute-r-script] Modul.

In diesem Modul akzeptiert mehrere Eingabewerte Datasets und das Ergebnis ist nur ein Dataset als Ausgabe. Sie können ein Skript R in den Parameter **R Skript** des [Ausführen R Skript] eingeben[ execute-r-script] Modul.

Sie zugreifen jede Eingabe Anschluss des Moduls mithilfe von Code ähnlich wie der folgende aus:

    dataset1 <- maml.mapInputPort(1)

[AZURE.INCLUDE [machine-learning-free-trial](../../includes/machine-learning-free-trial.md)]

##<a name="listing-all-currently-installed-packages"></a>Alle installierten Pakete auflisten

Die Liste der installierten Pakete ändern kann. Beim Abrufen der abgeschlossen, aktuellen Liste der installierten Pakete, einschließlich der Beschreibung der einzelnen Paket, geben Sie den folgenden Code in das [Ausführen R Skript] [ execute-r-script] Modul:

    out <- data.frame(installed.packages(,,,fields="Description"))
    maml.mapOutputPort("out")

Sendet die Liste der Pakete des [Ausführen R Skript] -Ausgang[ execute-r-script] Modul.
Zum Anzeigen der Paketliste verbinden ein Moduls Konvertierung, wie etwa [CSV-konvertieren] [ convert-to-csv] auf der linken Ausgabe des [R Skript ausführen] [ execute-r-script] Modul, führen Sie die experimentieren, klicken Sie auf die Ausgabe der Konvertierung Modul, und wählen Sie **herunterladen**. 
...
Pakete, die noch nicht aus einem bereitgestellte ML Studio Repository installiert sind, indem Sie mit den folgenden Befehlen im [R Skript ausführen] können auch importieren[ execute-r-script] Modul und ZIP-Paket archivieren:

    install.packages("src/my_favorite_package.zip", lib = ".", repos = NULL, verbose = TRUE)
    success <- library("my_favorite_package", lib.loc = ".", logical.return = TRUE, verbose = TRUE)

Stelle, an der die `my_favorite_package.zip` die ZIP-Datei mit Ihrem Paket enthält.
